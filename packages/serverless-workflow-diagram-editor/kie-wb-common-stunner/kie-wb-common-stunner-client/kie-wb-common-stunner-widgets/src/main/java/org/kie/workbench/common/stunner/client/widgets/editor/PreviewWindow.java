...
public class PreviewWindow {
    ...
    void onTogglePreviewEvent(@Observes TogglePreviewEvent event) {
        if (event.getEventType() == TogglePreviewEvent.EventType.TOGGLE) {
            show(event.getX(),
                 event.getY(),
                 event.getWidth(),
                 event.getHeight());
            ...
            if (null != previewRoot && previewRoot.isVisible()) {
                if (event.getEventType() == TogglePreviewEvent.EventType.HIDE) {
                    close();
                } else if (event.getEventType() == TogglePreviewEvent.EventType.RESIZE) {
                    translate(event.getX(),
                              event.getY(),
                              event.getWidth(),
                              event.getHeight());
                }
